...
def dump_json(path, data, language):
    with open(os.path.join(path,f"{language}.json"), "w+") as file:
        json.dump(data, file, indent=4, ensure_ascii=False,)
    pass

def run_percentage(files, split, language):
    ...
    dump_json(output_data_path,meta_data_information, language)
    print("min ",min_)
    print("max ",max_)
...
if __name__=="__main__":
    ...
    for language in languages:
        train_data_path = f"/srv/upadro/dataset/{language}/unseen_queries/train"
        test_data_path = f"/srv/upadro/dataset/{language}/unseen_queries/test"
        unique_query_test_data_path = f"/srv/upadro/dataset/{language}/unseen_queries/unique_query_test"
        val_data_path = f"/srv/upadro/dataset/{language}/unseen_queries/val"
        train_test_val = f"/srv/upadro/dataset/{language}/unseen_queries/train_test_val"
        ...
        for input_data_path in data_paths:
            ...
            run_percentage(files, split, language)

...
if __name__=="__main__":
    ...
    for language in languages:
        output_data_path = f"output/{language}/jsons/"
        files = []
        ...
        for file in files:
            ...
            output_file =  os.path.join("output", language, "relevant_jsons", f"{file_name}_relevant.json")
            with open(output_file, 'w') as file:
                json.dump(usable, file, indent=4, ensure_ascii=False,)
            meta_data_file = os.path.join("output", language, "analysis", "metadata_analysis.txt")

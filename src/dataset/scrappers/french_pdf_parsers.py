...
def make_csv(results):
    csv_file_output =  os.path.join("output","french.csv")
...
def convert_to_json(final_result, file_name = "results.json"):
    ...
    file_name = os.path.join("output","french","jsons", file_name)
    with open(file_name, "w+") as file:
        json.dump(json_result, file, ensure_ascii=False, indent=4)
        
if __name__ == "__main__":
    raw_data_path = "raw_data/french/"
    files = []
    ...
    for file in files:
        ...
        text_file_output =  os.path.join("output", "french","tests",f"french_results-{file_name}.txt")
        ...
        number_result_file_output =  os.path.join("output", "french", "french_number_results.txt")
        ...
        # unusable_docs =  os.path.join("output", "french", "unusable_docw_results.txt")
